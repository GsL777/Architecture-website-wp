...
function architecture_tooltip( $attr, $content = null ){//First parameter - the array that contains the attributes. Second parameter - is the content. Text wrapped around a shortcode. $content = null - default action not to trigger an error if [tooltip][/tooltip] is empty.
...
*/
	...
	//return HTML
	return '<span class="architecture-popover" data-toggle="popover" data-placement="' . $attr['placement'] . '" title="' . $attr['title'] . '" data-trigger="' . $attr['trigger'] . '" data-content="' . $attr['content'] . '">' . $content . '</span>';
...
add_shortcode( 'popover', 'architecture_popover' );//For function to work need to put $('[data-toggle="popover"]').popover(); to revealPosts function to main-jquery.js
...
function architecture_contact_form( $attr, $content = null ){//First parameter - the array that contains the attributes. Second parameter - is the content. Text wrapped around a shortcode. $content = null - default action not to trigger an error if [contact_form][/contact_form] is empty.
...
*/
	...
	return ob_get_clean();//files in architecture-contact-form.php, content-contact.php
}

add_shortcode( 'contact_form', 'architecture_contact_form' ); //First Variable - shortcode name, Second variable - the name of the function shortcode that must be called
